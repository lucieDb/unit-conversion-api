...
RSpec.describe Units::Converters::VolumeConverter do
  ...
  describe "#convert" do
    ...
    it "converts 0 LITERS to GALLONS" do
      expect(converter.convert(0, "LITERS", "GALLONS").round(2)).to eq(0.0)
    end

    it "converts negative LITERS to CUPS" do
      expect(converter.convert(-2, "LITERS", "CUPS").round(2)).to eq(-8.45)
    end

    it "converts LITERS to CUPS with mixed case units" do
      expect(converter.convert(1, "Liters", "cUpS").round(2)).to eq(4.23)
    end
    ...
    it "converts LITERS to LITERS without changing value" do
      expect(converter.convert(42, "LITERS", "LITERS").round(2)).to eq(42.0)
    end
    ...
    it "converts a very small value of TABLESPOONS to LITERS" do
      expect(converter.convert(0.0001, "TABLESPOONS", "LITERS").round(6)).to eq(0.000001)
    end
    ...
    it "converts TABLESPOONS to TABLESPOONS without changing value" do
      expect(converter.convert(42, "TABLESPOONS", "TABLESPOONS").round(2)).to eq(42.0)
    end
    ...
    it "converts CUBIC-INCHES to CUBIC-INCHES without changing value" do
      expect(converter.convert(42, "CUBIC-INCHES", "CUBIC-INCHES").round(2)).to eq(42.0)
    end
    ...
    it "converts CUBIC-FEET to CUBIC-FEET without changing value" do
      expect(converter.convert(42, "CUBIC-FEET", "CUBIC-FEET").round(2)).to eq(42.0)
    end
    ...
    it "converts negative GALLONS to LITERS" do
      expect(converter.convert(-3, "GALLONS", "LITERS").round(2)).to eq(-11.36)
    end

    it "converts a very large value of GALLONS to CUBIC-FEET" do
      expect(converter.convert(1_000_000, "GALLONS", "CUBIC-FEET").round(2)).to eq(133680.71)
    end
    ...
    it "converts GALLONS to GALLONS without changing value" do
      expect(converter.convert(42, "GALLONS", "GALLONS").round(2)).to eq(42.0)
    end

    it "raises error for unknown unit" do
      expect { converter.convert(1, "banana", "LITERS") }.to raise_error(ArgumentError, /Unknown volume unit/)
    end

    it "raises an error for unknown target unit" do
      expect { converter.convert(1, "LITERS", "tomatoes") }.to raise_error(ArgumentError, /Unknown volume unit/)
    end
  end
end
